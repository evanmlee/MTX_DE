...
#######====================== M6_lenient_MTX.R ======================#######
#M6_lenient_MTX is a wrapper script for MTX_model using feature-level DNA covariates. Lenient (feature-level) zero filtering is used. 

#Model specifcations: log transform, total sum scaling (TSS) normalization, fixed_effects of Phenotype (in metadata file).
#Model specifcations (continued): using per-feature DNA abundances in input_dnadata. 'lenient' rna_dna_filter 
...
input_dnadata <-file.path(input_dir,sprintf("%s.mgx_abunds.tsv",dataset_name)) #mgx_abunds (feature-level) for M6
fit_data <- MTXmodel(
