...
#######====================== M1_strict_MTX.R ======================#######
#M5_MTX is a wrapper script for MTX_model using no DNA covariates. Strict RNA/DNA filtering is used.

#Model specifcations: log transform, total sum scaling (TSS) normalization, fixed_effects of Phenotype (in metadata file).
#Model specifcations (continued): using per-taxon DNA abundances in input_dnadata. 'strict' rna_dna_filter 
...
# input_dnadata <-file.path(input_dir,sprintf("%s.mgx_bug_abunds.tsv",dataset_name)) #no input_dnadata for M1 model 
fit_data <- MTXmodel(
    ...
    normalization = 'TSS',
    standardize = FALSE,
    input_dnadata = 'none', #no input_dnadata for M1 
    rna_dna_flt = "strict",
    plot_heatmap = FALSE,
